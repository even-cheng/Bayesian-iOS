...
+ (NSDictionary *)getPropertys
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([propertyType hasPrefix:@"T@"] && [propertyType containsString:@"NSData"]){
...
- (BOOL)update
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        for (int i = 0; i < self.columeNames.count; i++) {
            ...
            [keyString appendFormat:@" %@=?,", proname];
            id value = [self valueForKey:proname];
            if ([proname isEqualToString:@"money"]) {
                NSLog(@"%@",value);
            }
            if (!value) {
                value = @"";
...
//            
//            //刷新聊天列表
//            [XNotificationCenter postNotificationName:xReloadMessageListNotification object:nil];
//        });
    }];
    return res;
}
        ...
        while ([resultSet next]) {
            ...
            for (int i=0; i< model.columeNames.count; i++) {
                 ...
                 if ([columeType isEqualToString:SQLTEXT]) {
                     ...
                     [model setValue:@([resultSet doubleForColumn:columeName]) forKey:columeName];
...
/** 通过sql语句查找数据 */
+ (NSArray *)findBySqlString:(NSString *)string
{
    DBHelper *jkDB = [DBHelper shareInstance];
    NSMutableArray *users = [NSMutableArray array];
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
